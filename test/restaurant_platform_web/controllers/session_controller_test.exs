defmodule RestaurantPlatformWeb.SessionControllerTest do
  ...
  @update_attrs %{
    ...
    start_time: ~U[2024-08-27 10:17:00Z],
    end_time: ~U[2024-08-27 10:17:00Z]
  }
  ...
  describe "create session" do
    test "renders session when data is valid", %{conn: conn} do
      ...
      assert %{
               "id" => ^id,
               "end_time" => "2024-08-26T10:17:00Z",
               "session_token" => "some session_token",
               "start_time" => "2024-08-26T10:17:00Z"
  ...
  describe "update session" do
    ...
    test "renders session when data is valid", %{conn: conn, session: %Session{id: id} = session} do
      ...
      assert %{
               "id" => ^id,
               "end_time" => "2024-08-27T10:17:00Z",
               "session_token" => "some updated session_token",
               "start_time" => "2024-08-27T10:17:00Z"
             } = json_response(conn, 200)["data"]
    end
